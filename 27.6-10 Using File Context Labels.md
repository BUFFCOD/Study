...
For RHCSA you need to know 3 thing fcontext labels,ports, and booleans.

example of setting a boolean:
setsebool -P httpd_can_network_connect on # allow httpd to connect to the network

example of setting a port:
semanage port -a -t http_port_t -p tcp 8080 # allow httpd to use port 8080

example of setting a fcontext label:
semanage fcontext -a -t httpd_sys_content_t "/my/web(/.*)?" # allow httpd to access files in /my/web and subdirectories

and to troubleshoot an issue:

setenforce 0 # set to permissive mode to confirm SELinux is the issue
grep AVC /var/log/audit/audit.log # find relevant messages in the audit log
sealert -a /var/log/audit/audit.log # get suggestions on how to fix the issue
